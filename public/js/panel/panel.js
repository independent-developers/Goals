...
;(function($) {
	...
	twitch.onAuthorized(function(auth) {
		...
		manager.goals.fetch(channelID).then(function() {
			const goals_local_without_empty_title = goals_local.filter(goal => {
			...
			if (isBroadcaster === false) {
				goals_local_without_empty_title.forEach(goal => {
					perform_element(goal.key, goal.title, goal.isChecked, false)
				})
			}
			...
			if (isBroadcaster === false && goals_local.length !== 0) {
				const label =
					goals_local_without_empty_title.length === 1 ? ' goal' : ' goals'
				$('.btn_add').html(goals_local_without_empty_title.length + label)
			}
		...
		if (isBroadcaster === false && listenFirstTimeOnly === true) {
			...
			twitch.listen('broadcast', function(target, contentType, message) {
				...
				// Do not share goals with empty title
				const goalsWithoutEmptyTitle = response.goals.filter(goal => {
					return goal.title.length !== 0
				})

				// Preliminary: Check if goals exist on message
				if (goalsWithoutEmptyTitle && goalsWithoutEmptyTitle !== undefined) {
					...
					goals_local = goalsWithoutEmptyTitle

					// Add all goals on user interface
					goalsWithoutEmptyTitle.forEach(goal => {
						perform_element(goal.key, goal.title, goal.isChecked, false)
					})
				}

				// Perform bottom for viewer
				if (goalsWithoutEmptyTitle.length !== 0) {
					const label = goalsWithoutEmptyTitle.length === 1 ? ' goal' : ' goals'
					$('.btn_add').html(goalsWithoutEmptyTitle.length + label)
				} else {
					$('.btn_add').html('')
				}
			})
...
function perform_element(key, title, isChecked, isCreated) {
	...
	let element =
		...
		'</span>' +
		'</label>' +
		'<span class="title"> <textarea class="title" id="title_' +
		key +
		'" rows="2" cols="30" data-limit-rows="true" placeholder="Enter your goal...">' +
		title +
		'</textarea> </span>' +
		'<span class="trash ' +
		key +
	...
	if (isBroadcaster === true && isCreated === true) {
		manager.goals.create(channelID, [
			{ key: key, title: title, isChecked: isChecked },
		])
	}
}
...
async function update(key, title, isChecked) {
	await manager.goals.update(channelID, [
		{ key: key, title: title, isChecked: isChecked },
	])
