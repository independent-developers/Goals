...
function create(request, h) {
    ...
    console.log(':: creating goal..', request.params)

    if(!userId) return Boom.badRequest('Missing userId');

    const goalCreated = DAL.create(request.payload, userId);
    return h.response({
        success: true,
...
function update(request, h) {
    ...
    if(!goalId) return Boom.badRequest('Missing goalId');
    DAL.update("goals", request.payload, goalId);
    return {
      success: true,
